...
public class NurseRecord_ReceiveActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_nurse_record__recieve);

        toolbar();

        RecyclerView recyclerView=findViewById(R.id.rylView_foster_petList);
        LinearLayoutManager layoutManager=new LinearLayoutManager(NurseRecord_ReceiveActivity.this);
        recyclerView.setLayoutManager(layoutManager);
        adapter = new FosterListAdapter(NurseRecord_ReceiveActivity.this);
    ...
    private void loadListData(){
        HttpUtil.sendGet("/nurse/reRecords?userID="+ LogInfo.getUser(NurseRecord_ReceiveActivity.this).getUserID(),new Callback() {
            @Override
            public void onFailure(Call call, IOException e) {
                NurseRecord_ReceiveActivity.this.runOnUiThread(()->{
                    Toast.makeText(NurseRecord_ReceiveActivity.this.getApplicationContext(),"加载失败",Toast.LENGTH_SHORT).show();
                });
            }
